...
async function renameCollection() {
  ...
  try {
    console.log(
      `Attempting to rename collection '${OLD_COLLECTION_NAME}' to '${NEW_COLLECTION_NAME}'...`
    );

    // Check if the old collection exists
    const collections = await db
      .listCollections({ name: OLD_COLLECTION_NAME })
      .toArray();
    if (collections.length === 0) {
      console.log(
        `Source collection '${OLD_COLLECTION_NAME}' does not exist. Skipping rename.`
      );
    ...
    const targetCollections = await db
      .listCollections({ name: NEW_COLLECTION_NAME })
      .toArray();
    if (targetCollections.length > 0) {
      console.warn(
        `Warning: Target collection '${NEW_COLLECTION_NAME}' already exists. Renaming might fail or cause issues. Consider dropping it first if this rename is intended to replace it.`
      );
    ...
    console.log(
      `Successfully renamed collection '${OLD_COLLECTION_NAME}' to '${NEW_COLLECTION_NAME}'.`
    );
  } catch (error) {
    ...
    if (error.codeName === "NamespaceExists") {
      console.error(
        `Hint: The collection '${NEW_COLLECTION_NAME}' already exists. Drop it first if you intend to overwrite it.`
      );
    }
  } finally {
