...
const StandardizedLocation = mongoose.model('StandardizedLocation', standardizedLocationSchema);
  ...
  try {
    ...
    for (const subject of subjects) {
      ...
      for (const location of subject.locations) {
        if (!location.geo || location.geo.length !== 2 || location.id === null || location.id === undefined) {
        ...
        if (!standardizedLocationsMap.has(key)) {
          standardizedLocationsMap.set(key, {
            representative: { // Store the first location encountered for this rounded coord
              id: location.id,
              name: location.name,
    ...
    console.log(`Found ${standardizedLocationsMap.size} unique standardized locations based on rounded coordinates.`);
    ...
    console.log('Preparing bulk upsert operations for standardized_locations collection...');
    const bulkOps = [];
