...
async function enrichLocations() {
  ...
  try {
    ...
    while (true) {
      // Loop until no more locations need processing
      batchNumber++;
      console.log(`\n--- Starting Batch ${batchNumber} ---`);
      const remainingCount = await StandardizedLocation.countDocuments({
      ...
      const locationsToEnrich = await StandardizedLocation.find({
        gmap_processed: { $ne: true },
      })
        .limit(BATCH_SIZE)
      ...
      console.log(
        `Processing ${locationsToEnrich.length} locations in batch ${batchNumber}...`
      );
      const bulkOps = [];
      ...
      for (const loc of locationsToEnrich) {
        ...
        if (
          loc._id == null ||
          typeof loc.lat_precise !== "number" ||
          typeof loc.lng_precise !== "number"
        ) {
          console.warn(
            `   ID ${
              loc._id
            }: Skipping location with invalid data: ${JSON.stringify(
              loc
        ...
        try {
          ...
          if (
            ...
            console.warn(
              `   ID ${loc._id}: No results or error from API: ${response.data.status}`
            );
          ...
          console.error(
            `   ID ${loc._id}: Error during API call: ${error.message}. Marking as processed.`
          );
        ...
        if (
          batchProcessedCount % 10 === 0 ||
          batchProcessedCount === locationsToEnrich.length
        ) {
          console.log(
            `   Batch ${batchNumber}: Processed ${batchProcessedCount}/${locationsToEnrich.length}... (Total processed overall: ${totalProcessed})`
          );
        }
      ...
      if (bulkOps.length > 0) {
        console.log(
          `Executing ${bulkOps.length} updates for batch ${batchNumber}...`
        );
        const bulkResult = await StandardizedLocation.bulkWrite(bulkOps);
        console.log(
          `Batch ${batchNumber} update result: Matched: ${bulkResult.matchedCount}, Modified: ${bulkResult.modifiedCount}`
        );
      } else {
        console.log(
          `Batch ${batchNumber}: No updates to execute (all skipped?).`
        );
      }
    ...
    console.error(
      "An unrecoverable error occurred during the enrichment process:",
      error
    );
    process.exit(1);
  } finally {
