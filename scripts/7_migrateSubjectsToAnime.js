...
async function migrateSubjectsToAnime() {
  ...
  try {
    console.log(
      `Starting migration from '${SOURCE_COLLECTION}' to '${TARGET_COLLECTION}'...`
    );
    console.log(
      `This will overwrite the '${TARGET_COLLECTION}' collection if it already exists.`
    );
    ...
    console.log(
      `Successfully migrated data to the '${TARGET_COLLECTION}' collection.`
    );
    ...
    console.log(
      `Source collection '${SOURCE_COLLECTION}' count: ${sourceCount}`
    );
    console.log(
      `Target collection '${TARGET_COLLECTION}' count: ${targetCount}`
    );
    if (sourceCount !== targetCount) {
      console.warn(
        "Warning: Source and target document counts do not match. Check the pipeline logic if this was unexpected."
      );
    }
  } catch (error) {
