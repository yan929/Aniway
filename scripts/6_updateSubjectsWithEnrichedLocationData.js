...
const standardizedLocationSchema = new mongoose.Schema(
  ...
  { collection: "standardized_locations", read: "secondaryPreferred" }
); // Read from secondary if possible
const StandardizedLocation = mongoose.model(
  "StandardizedLocationLookup",
...
const subjectSchema = new mongoose.Schema(
  ...
  { collection: "subjects", strict: false }
); // Allow other fields
  ...
  try {
    while (hasMoreSubjects) {
      ...
      if (standardizedIds.size === 0) {
        console.log(
          "No standardized location IDs found in this batch. Skipping enrichment for this batch."
        );
      ...
      const standardizedLocations = await StandardizedLocation.find({
        ...
        .select("_id gmap_formatted_addresses")
        .lean();
      ...
      console.log(
        `Fetched ${addressMap.size} standardized locations for lookup.`
      );
      ...
      if (bulkOps.length > 0) {
        ...
        console.log(
          `Batch ${page} update result: Matched: ${result.matchedCount}, Modified: ${result.modifiedCount}`
        );
        if (result.hasWriteErrors()) {
          console.warn(
            `Batch ${page} encountered write errors:`,
