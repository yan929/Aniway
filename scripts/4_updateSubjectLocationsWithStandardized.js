...
const subjectSchema = new mongoose.Schema(
  {
    ...
    locations_info: [
      // New top-level array for standardized references
      {
...
); // Use timestamps if they exist or you want them
...
async function main() {
  ...
  try {
    ...
    for (const doc of standardizedDocs) {
      ...
      if (
        doc.lat_rounded != null &&
        doc.lng_rounded != null &&
        doc._id != null &&
        doc.name != null &&
        doc.lat_precise != null &&
        doc.lng_precise != null
      ) {
        ...
        console.warn(
          `Skipping standardized location doc with missing critical fields: ${JSON.stringify(
            doc
          )}`
        );
      }
    ...
    console.log(
      `Loaded ${standardizedMap.size} standardized locations into map.`
    );

    if (standardizedMap.size === 0) {
      console.error(
        "Standardized locations map is empty. Cannot proceed. Run standardizeLocations.js first."
      );
      return;
    }
    ...
    for (const subject of subjects) {
      ...
      for (const originalLoc of originalLocations) {
        if (
          ...
          !originalLoc.id ||
          !originalLoc.geo ||
          originalLoc.geo.length !== 2
        ) {
          console.warn(
            `Subject ${
              subject._id
            }: Skipping invalid location object: ${JSON.stringify(originalLoc)}`
          );
          continue;
        }
        ...
        if (isNaN(roundedLat) || isNaN(roundedLng)) {
          console.warn(
            `Subject ${
              subject._id
            }: Skipping location due to non-numeric geo: ${JSON.stringify(
              originalLoc
            )}`
          );
          continue;
        ...
        if (representativeLoc) {
          ...
          console.warn(
            `Subject ${subject._id}: No standardized representative found for key ${key} (original location ID: ${originalLoc.id}).`
          );
        }
      }
    ...
    console.log(
      `Prepared ${bulkOps.length} bulk update operations for subjects.`
    );
