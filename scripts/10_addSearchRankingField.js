require('dotenv').config({ path: require('path').resolve(__dirname, '../backend/.env') }); // Use backend .env for aniway DB URI
...
async function addSearchRankingField() {
    ...
    let connection;
    try {
        connection = await mongoose.createConnection(TARGET_MONGODB_URI).asPromise();
        ...
        for (const collectionName of COLLECTIONS_TO_UPDATE) {
            ...
            console.log(`Adding field '${FIELD_TO_ADD}: ${DEFAULT_VALUE}' to documents in '${collectionName}' where it doesn't exist...`);

            const result = await collection.updateMany(filter, updateDoc);

            console.log(`Update result for '${collectionName}':`);
            console.log(`  Documents matched (missing the field): ${result.matchedCount}`);
            console.log(`  Documents modified: ${result.modifiedCount}`);
            if (result.upsertedCount > 0) {
                 console.log(`  Documents upserted: ${result.upsertedCount}`); // Should be 0
            }
            if (!result.acknowledged) {
                console.warn(`  Warning: Update operation for '${collectionName}' was not acknowledged.`);
            }
        }
        ...
        console.error('\nAn error occurred during the field addition process:', error);
    } finally {
        if (connection) {
