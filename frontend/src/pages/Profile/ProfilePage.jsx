  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch user trips
        const tripsResponse = await fetch(`${API_BASE_URL}/api/tplan`, {
          credentials: 'include'
        });
        ...
        if (tripsResponse.ok) {
          ...
          // Even if trips fetch fails, continue showing the page
          console.error('Failed to fetch trips:', tripsResponse.statusText);
        }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {trips.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm p-8 mb-8 text-center">
              <p className="text-gray-500">Your active trips will appear here.</p>
            </div>
          ) : (
            <div className="space-y-4 mb-8">
              {trips.map(trip => (
                <div 
                  ...
                  onClick={() => navigate(`/tripplanner?trip=${trip._id || trip.id}`)}
                >
                  <h3 className="font-semibold text-lg">{trip.title || 'Untitled Trip'}</h3>
                  {trip.content && trip.content.length > 0 && (
                    <div className="text-sm text-gray-500 mt-1">
                      <span>
                        {new Date(trip.content[0].date).toLocaleDateString()} - 
                        {new Date(trip.content[trip.content.length - 1].date).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                </div>
          ...
          <h2 className="text-2xl font-bold text-center mb-8">Completed trips</h2>
